...
## 2026-09-26 — visible burrow and intact grazing profile

Candidate for visual review; not a declaration of commercial animation acceptance.

- Replaced the firefish's small shadow-like oval with `BurrowPortal`: rear sand wall, dark aperture, foreground raised rim, and seven short-lived sand grains. Rear geometry is drawn after the backdrop and before the fish; the rim is above the fish. The opening persists while occupied or empty.
- Entry lasts 0.55 seconds: bend toward the opening above the sand, fold outer fins, then pass nose-first through the aperture. Emergence takes 1.3 seconds, head-first, leveling only above the rim. The shader clips inside the local opening, not across the entire flat sand plane. No simulation state or RNG writes.
- Claude's backend commit `ff4f767` now puts grazing tangs half a scaled body length from their contact point. The frontend retains projection 1 and the approved side-view asset. Regression verifies that this reach requires essentially zero compensating offset. A generated yaw-pose experiment was rejected after rendered inspection showed duplicated eyes during cross-fades; it is confined to ignored artifacts and excluded from export.
- Tests: `test_reef_animation` 46, `test_frontend` 80, `test_presentation` 29, all passing. Checks include correct portal draw layers, entry head orientation, local clipping, hidden tail clearance, complete return, and read-only snapshots/random state.
- Review files: `artifacts/portal-volume-review/purple-firefish-comparison.mp4` (24 s, 1.65×, tap at 8 s, entry and emergence) and `yellow-tang-current.mp4` (12 s, current side-on grazing). Firefish upper panel uses frontend `0ce038c`; both panels consume the same current backend. Tang video deliberately shows current output only: replaying the old rig against the already-fixed backend does not reproduce the original bad contact distance.
- Reproduction: archive the baseline rig/shader as described in the artifact README, then run `tools/natural_motion_review.gd -- --fix-review --output=res://artifacts/portal-volume-review/`. The script creates an isolated world and never loads/saves the user's world.
- Visual inspection included frames before the bend, nose at rim, partly occluded body, fully hidden fish, emerging head, and fully restored hover. A first-pass layering error (rear wall behind the backdrop) was found in these renders and fixed before final recording.
- Still pending: user's visual judgement and sustained packaged/thermal acceptance. The tang's general turn still uses the original side-view projection; the rejected cross-fade experiment does not count as a completed volumetric turn.
- Bounded performance check: 38 s isolated source preview, 960×540, 1,138 rendered frames / 37.885 s; `ps` samples after 5 s warmup averaged 12.01% CPU (one-core scale), peak RSS 255.17 MiB. This is a short smoke measurement, not 30-minute packaged or thermal validation. Raw samples are in `artifacts/portal-volume-review/performance.json`.

## 2026-09-26 — interrupted shelter transitions / turn research

**Implemented:** firefish presentation now completes the current entry/emergence arc before consuming the latest `extend` request. Previously, changing the target halfway through a visible arc selected the opposite pose formula and flipped the fish by about PI radians in one frame. Requests are coalesced, not queued indefinitely; maximum additional wait is the remaining 0.55 s entry or 1.3 s emergence. This visual buffer does not alter ecological state. Hide/wake regression cases were observed failing before the fix and passing afterwards. Current totals: animation 50, frontend 80, presentation 29, all pass.

`artifacts/portal-transition-review/interruption-comparison.mp4` shows both directions of interruption at 1× and 1.65×, eight seconds. Upper row is rig `5068bab`; lower row uses the buffered transition. Frame 38→39 demonstrates the old visible 180-degree flip and its removal. Reproduce by saving `git show 5068bab:stream/scripts/reef_rig.gd` to `artifacts/portal-transition-review/before_rig.gd`, removing its `class_name ReefRig` line, then running `tools/portal_transition_review.gd`. No world or save is created by this probe.

**Not accepted / not integrated:** tang yaw experiments in `tools/art_candidates/tang_volume_candidate.gd` and `tang_volume_body.gdshader`. A single approved side-view texture is projected onto an analytical convex body to retain thickness without sprite cross-fades. The two-surface mesh approach was rejected after continuous playback exposed cracks that static angle sheets missed. Analytical ray projection removes those cracks, but the side-view eye texture becomes a dark band at the frontal angle and cannot supply the missing frontal facial design. This remains a diagnostic candidate, excluded by the `tools/*` export rule. `tools/tang_turn_review.gd` is a repeatable normal/1.65× full-turn probe, not a quality acceptance test. A proper frontal design and landmark-aligned transition remain required; do not treat finite body width alone as success.

**Goal remains incomplete:** frontal turns for all four species, complete per-species visual acceptance, final integrated motion/performance review, and Claude's formal packaged 30-minute validation. The original commercial-quality objective is unchanged.
