...
Breeding requires mature mates, sufficient reserves, a species-specific cooldown and a resource-dependent probability. A female shrimp that meets the condition becomes berried for five simulated days (user decision 2026-09-23); her brood hatches when that period ends, and the brood cost is taken then, so her reserves at hatching decide how many young survive. The cooldown counts from the start of the brood, and she cannot start another while berried. If she dies while berried the brood is lost. Fish breed on the spot as before. Each shrimp also carries a fixed appearance-only `tint`, inherited roughly from the mother; it never affects ecology and is derived from the world seed and id, not from the simulation's random streams. Space permits up to eight shrimp, five threadfins and five hatchetfish. Excess offspring explicitly disperse downstream. The defensive active-individual limit is 24; ordinary new worlds remain at or below the combined habitat limit of 18. Adults do not randomly depart. Low local species counts allow rescue arrivals; ordinary immigration is much less frequent.
