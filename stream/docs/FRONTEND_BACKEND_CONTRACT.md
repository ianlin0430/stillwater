...
> **2026-09-26 動畫候選**：已接自然游動欄位並依最新「流暢優先」指示加緩衝；不改 backend 軌跡。四種 1.65× 修前修後短片、測試與驗收狀態見 [NATURAL_MOTION_FRONTEND_REVIEW.md](NATURAL_MOTION_FRONTEND_REVIEW.md)。使用者動態核可仍待確認。

> **2026-09-25 最新接線**：已使用 Claude 的四物種／12 隻開局、加寬洞距與黃金吊接觸距離。新增 live `ate` → 對應 id 的短暫咬食；不再從飼料消失推測誰吃。backend 花園鰻移除已完成，以下「過渡期間仍有花園鰻」為歷史。自然事件證據、測試與封裝自查見 REEF_FRONTEND_REVIEW 最上方。

> **2026-09-25 覆蓋決定：花園鰻先不做。** 前端僅顯示四種魚，Claude 需同步後端名單及既有個體退出策略。前端不刪存檔個體；過渡期間 backend 仍有花園鰻。詳見 REEF_FRONTEND_REVIEW 最上方。

> **2026-09-24 最新前端進度**：使用者在替換 03 為黃三角倒吊後要求依 Claude 計畫完成動畫，並再要求更細緻。目前五種礁岩角色、背景與互動已接入 source；含指定 A3 紫雷達。內部畫面依最新細節需求調為 960×540（世界座標仍 1280×720），維持 30 FPS。完整結果與 Claude 接手事項見 [REEF_FRONTEND_REVIEW.md](REEF_FRONTEND_REVIEW.md)。以下歷史淡水／無餵食契約由最新 CODEX_FRONTEND_PLAN §3.6–3.7 與 BACKEND_SNAPSHOT_EVENTS 取代；正式封裝與長測尚未完成。
