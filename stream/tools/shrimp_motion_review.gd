...
const OUT="res://artifacts/shrimp-review-b/"
...
func capture(name: String) -> void:
	...
	if name.begins_with("frames/"):
		view.get_texture().get_image().save_jpg(OUT+name+".jpg",0.96)
	else:
...
func run() -> void:
	...
	canvas.add_child(bg)
	old=preload("res://tools/art_candidates/shrimp_candidate.gd").new()
	fresh=Candidate.new()
	...
	for a: Dictionary in cases:
		...
		for zoom: float in [1.0,1.65]:
			...
			title.text="%s  |  %s  |  LEFT: previous A   RIGHT: motion B" % [a.name,"normal" if zoom==1 else "1.65x"]
	...
	old.apply_identity(cases[0],0)
	old.berried=false
	old.molting=false
	for frame in 480:
		var zoom: float=1.0 if frame<240 else 1.65
		canvas.scale=Vector2.ONE*zoom
		canvas.position=Vector2.ZERO if zoom==1 else Vector2(640,360)-Vector2(640,500)*zoom
		var section: int=(frame%240)/60
		title.text=["Walking","Swimming","Grazing","Turning"][section]+" | A / B | %sx | scripted rig study" % str(zoom)
		for rig: SwimmerRig in [old,fresh]:
			if frame%240==0:
				rig.position=Vector2(485 if rig==old else 775,651)
				rig.facing=1
				rig.face_target=1
				rig.tail_facing=1
				rig.reset_contact()
			if frame%240==120:
				rig.position.y=651
				rig.reset_contact()
			if section==1: rig.position.y-=5.0/30
