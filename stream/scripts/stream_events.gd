...
func accept(event: Dictionary, snapshot: Dictionary) -> void:
	...
	elif event.kind in ["molt","dispersal"]:
		if ghosts.size()>=LIMIT: return
		var rig:=SwimmerRig.new()
		rig.species=actor.species
		rig.sex=actor.sex
		rig.individual_id=actor.id
		rig.body_scale=0.75 if event.kind=="molt" else 0.32
		rig.scale=Vector2.ONE*rig.body_scale
		rig.facing=actor.direction if event.kind=="molt" else 1.0
		rig.face_target=rig.facing
		rig.tail_facing=rig.facing
		rig.exuvia=event.kind=="molt"
		rig.activity="Resting" if event.kind=="molt" else "Swimming"
		var origin:=Vector2(event.get("x",actor.x),event.get("y",actor.y))
		if actor.species=="shrimp": origin.y-=12*rig.body_scale
		rig.position=origin
		rig.modulate=Color(0.88,0.91,0.74,0.32) if event.kind=="molt" else Color(0.55,0.65,0.59,0.5)
		add_child(rig)
		ghosts.append({"rig":rig,"age":0.0,"origin":origin,"kind":event.kind,"until":event.get("until",snapshot.elapsed),"expired":0.0})

func advance(delta: float, simulation_time: float, rigs: Dictionary) -> void:
	...
	for effect: Dictionary in ghosts.duplicate():
		...
		if effect.kind=="dispersal":
			...
			if simulation_time>=effect.until: effect.expired+=maxf(delta,0)
			rig.modulate.a=0.32*maxf(0,1-effect.expired/2)
			if effect.expired>=2:
				rig.queue_free()
				ghosts.erase(effect)
